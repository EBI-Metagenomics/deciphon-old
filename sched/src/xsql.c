...
{
    assert(col >= 0);
    if (sqlite3_bind_double(stmt, col + 1, val)) return EFAIL;
    return DONE;
}
...
{
    assert(col >= 0);
    if (sqlite3_bind_int64(stmt, col + 1, val)) return EFAIL;
    return DONE;
}
...
{
    if (sqlite3_open(filepath, db)) return EFAIL;
...
{
    if (sqlite3_close(db)) return EFAIL;
    return DONE;
}
...
enum rc xsql_exec(struct sqlite3 *db, char const *sql, xsql_callback cb,
                  void *cb_arg)
{
    if (sqlite3_exec(db, sql, cb, cb_arg, 0)) return EFAIL;
    return DONE;
}
...
{
    if (sqlite3_prepare_v2(db, sql, -1, stmt, 0)) return EFAIL;
    return DONE;
}

enum rc xsql_reset(struct sqlite3_stmt *stmt)
{
    if (sqlite3_reset(stmt)) return EFAIL;
    return DONE;
}
...
enum rc xsql_step(struct sqlite3_stmt *stmt)
{
    ...
    if (code == SQLITE_ROW) return NEXT;
    return EFAIL;
}
