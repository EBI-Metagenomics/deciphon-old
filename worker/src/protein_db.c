                            ...
                            imm_float *epsilon)
{
    if (float_bytes == 4)
    {
        ...
        if (!cmp_read_float(cmp, &e))
            return error(RC_EIO, "failed to read epsilon");
        *epsilon = (imm_float)e;
    }
    else
    {
                             ...
                             imm_float epsilon)
{
    if (float_bytes == 4)
    {
        ...
        if (!cmp_write_float(cmp, e))
            return error(RC_EIO, "failed to write epsilon");
    }
    else
    {
...
static enum rc read_entry_dist(struct cmp_ctx_s *cmp, enum entry_dist *edist)
{
    ...
    if (!cmp_read_u8(cmp, &val))
        return error(RC_EIO, "failed to read entry distribution");
    *edist = val;
    return RC_DONE;
}
...
static enum rc write_entry_dist(struct cmp_ctx_s *cmp, enum entry_dist edist)
{
    if (!cmp_write_u8(cmp, (uint8_t)edist))
        return error(RC_EIO, "failed to write entry distribution");
    return RC_DONE;
}
