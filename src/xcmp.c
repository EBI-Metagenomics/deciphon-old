...
static bool read(struct lip_file *ctx, void *data, size_t sz)
{
...
// static bool skip(struct lip_file *ctx, size_t count)
// {
//     return fseeko((FILE *)ctx->buf, (off_t)count, SEEK_CUR) != -1;
// }

static size_t write(struct lip_file *ctx, const void *data, size_t count)
{
    struct xcmp *x = ctx->buf;
