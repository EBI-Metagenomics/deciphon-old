...
void loopio_init(struct loopio *l, struct uv_loop_s *loop,
                 loopio_onread_fn_t *onread_cb, loopio_onerror_fn_t onerror_cb,
                 void *arg)
{
    l->loop = loop;
    fio_init(&l->input, loop, &input_onopen, &input_onclose, l);
    fio_init(&l->output, loop, &output_onopen, &output_onclose, l);
    reader_init(&l->reader, loop, &reader_onerror, &reader_onread,
                &reader_onclose, l);
    writer_init(&l->writer, loop, &writer_onclose, l);
