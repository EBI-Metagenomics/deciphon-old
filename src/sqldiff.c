...
static inline void *sqlite3_malloc_u(unsigned size)
{
    return sqlite3_malloc((int)size);
}

static inline void *sqlite3_malloc_ul(unsigned long size)
{
    return sqlite3_malloc((int)size);
}

static inline void *sqlite3_realloc_u(void *ptr, unsigned size)
{
    return sqlite3_realloc(ptr, (int)size);
}

static inline void *sqlite3_realloc_ul(void *ptr, unsigned long size)
{
    return sqlite3_realloc(ptr, (int)size);
}

#define sqlite3_malloc_generic(x)                                              \
    _Generic((x), int                                                          \
             : sqlite3_malloc, unsigned                                        \
             : sqlite3_malloc_u, unsigned long                                 \
             : sqlite3_malloc_ul)(x)

#define sqlite3_realloc_generic(x, y)                                          \
    _Generic((y), int                                                          \
             : sqlite3_realloc, unsigned                                       \
             : sqlite3_realloc_u, unsigned long                                \
             : sqlite3_realloc_ul)(x, y)
...
static void strPrintf(Str *p, const char *zFormat, ...)
{
    ...
    for (;;)
    {
        ...
        p->z = sqlite3_realloc_generic(p->z, p->nAlloc);
...
)
{
    ...
    if (g.bSchemaPK == 0)
    {
        ...
        */
        ...
        {
            if (sqlite3_stricmp((const char *)sqlite3_column_text(pStmt, 3),
                                "pk") == 0)
            {
                zPkIdxName =
    ...
    *pnPKey = nPK;
    naz = nPK;
    az = sqlite3_malloc_generic(sizeof(char *) * ((size_t)nPK + 1));
    if (az == 0) runtimeError("out of memory");
    memset(az, 0, sizeof(char *) * ((size_t)nPK + 1));
    if (g.bSchemaCompare)
    {
    ...
    while (SQLITE_ROW == sqlite3_step(pStmt))
    {
        ...
        {
            if (!g.bSchemaCompare ||
                !(strcmp(sid, "rootpage") == 0 || strcmp(sid, "name") == 0 ||
                  strcmp(sid, "type") == 0))
            {
                az = sqlite3_realloc_generic(az, sizeof(char *) *
                                                     ((size_t)naz + 2));
                if (az == 0) runtimeError("out of memory");
                az[naz++] = sid;
            }
    ...
    ** avoids extraneous diffs against the schemas due to rowid variance. */
    if (az[0] == 0)
    {
        ...
        for (i = 0; i < (int)(sizeof(azRowid) / sizeof(azRowid[0])); i++)
        {
            for (j = 1; j < naz; j++)
            {
...
static void diff_one_table(const char *zTab, FILE *out)
{
    ...
    {
        if (!sqlite3_table_column_metadata(g.db, "main", zTab, 0, 0, 0, 0, 0,
                                           0))
        {
...
** commands.
*/
static int rbuDeltaCreate(const char *zSrc,    /* The source or pattern file */
...
)
{
    ...
    */
    ...
    memset(collide, -1, (size_t)nHash * sizeof(int));
    ...
    while (base + NHASH < lenOut)
    {
        ...
        while (1)
        {
            ...
            while (iBlock >= 0 && (limit--) > 0)
            {
                ...
                **
                ** Look for a region in zOut that matches zSrc. Anchor the
                *search
                ** at zSrc[iSrc] and zOut[base+i].  Do not include anything
                *prior to
                ** zOut[base] or after zOut[outLen] nor anything after
                *zSrc[srcLen].
                **
                ** Set cnt equal to the length of the match and set ofst so that
                ** zSrc[ofst] is the first element of the match.  litsz is the
                *number
                ** of characters between zOut[base] and the beginning of the
                *match.
                ** sz will be the overhead (in bytes) needed to encode the copy
                ** command.  Only generate copy command if the overhead of the
                ** copy command is less than the amount of literal text to be
                *copied.
                */
                int cnt, ofst, litsz;
                int j, k, x, y;
                ...
                ** the number of characters that match */
                ...
                /* Beginning at iSrc-1, match backwards as far as we can.  k
                 **count
                 *** the number of characters that match */
                for (k = 1; k < iSrc && (unsigned int)k <= i; k++)
                {
                    if (zSrc[iSrc - k] != zOut[base + i - (unsigned)k]) break;
                }
                k--;
                ...
                ofst = iSrc - k;
                cnt = j + k + 1;
                litsz = (int)(i - (unsigned)k); /* Number of bytes of literal
                                                   text before the copy */
                /* sz will hold the number of bytes needed to encode the
                 **"insert
                 *** command and the copy command, not counting the "insert" tex
                 */
                sz = digit_count((int)i - k) + digit_count(cnt) +
                     digit_count(ofst) + 3;
            ...
            */
            if (bestCnt > 0)
            {
                ...
                base += (unsigned)bestCnt;
                putInt((unsigned)bestCnt, &zDelta);
                *(zDelta++) = '@';
                putInt((unsigned)bestOfst, &zDelta);
                *(zDelta++) = ',';
...
static void strPrintfArray(Str *pStr,        /* String object to append to */
                           const char *zSep, /* Separator string */
                           const char *zFmt, /* Format for each entry */
                           char **az,
                           int n /* Array of strings & its size (or -1) */
)
{
    int i;
...
static void getRbudiffQuery(const char *zTab, char **azCol, int nPK,
                            int bOtaRowid, Str *pSql)
{
    int i;
...
static void rbudiff_one_table(const char *zTab, FILE *out)
{
    ...
    ** in the data_xxx table. */
    ...
    while (sqlite3_step(pStmt) == SQLITE_ROW)
    {
        ...
        ** printed again.  */
        ...
        else
        {
            ...
            {
                int bDone = 0;
                if (i >= nPK && sqlite3_column_type(pStmt, i) == SQLITE_BLOB &&
                    sqlite3_column_type(pStmt, nCol + 1 + i) == SQLITE_BLOB)
                {
                    ...
                    aDelta = sqlite3_malloc_generic(nFinal + 60);
                    nDelta = rbuDeltaCreate(aSrc, (unsigned)nSrc, aFinal,
                                            (unsigned)nFinal, aDelta);
                    if (nDelta < nFinal)
                    {
                        int j;
                        fprintf(out, "x'");
    ...
    {
        Str cnt = {0, 0, 0};
        strPrintf(&cnt, "INSERT INTO rbu_count VALUES('data_%q', %d);", zTab,
                  nRow);
        fprintf(out, "%s\n", cnt.z);
        strFree(&cnt);
    }
...
static void putsVarint(FILE *out, sqlite3_uint64 v)
{
    ...
    else
    {
        ...
        p[9] &= 0x7f;
        fwrite(p + n + 1, 9 - (unsigned)n, 1, out);
    }
}
...
static void changeset_one_table(const char *zTab, FILE *out)
{
    ...
    {
        nCol++;
        azCol = sqlite3_realloc_generic(azCol, sizeof(char *) * (size_t)nCol);
        if (azCol == 0) runtimeError("out of memory");
        aiFlg = sqlite3_realloc_generic(aiFlg, sizeof(int) * (size_t)nCol);
        ...
        if (i > 0)
        {
            ...
            {
                nPk = i;
                aiPk = sqlite3_realloc_generic(aiPk, sizeof(int) * (size_t)nPk);
    ...
    if (nCol > nPk)
    {
        ...
        for (i = 0; i < nPk; i++)
        {
            strPrintf(&sql, "%s A.%s=B.%s", zSep, azCol[aiPk[i]],
                      azCol[aiPk[i]]);
            zSep = " AND";
        }
        zSep = "\n   AND (";
...
*/
static void module_name_func(sqlite3_context *pCtx, int nVal,
                             sqlite3_value **apVal)
{
    const char *zSql;
    char zToken[32];
...
*/
static const char *all_tables_sql(void)
{
    if (g.bHandleVtab)
    {
        int rc;
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (z[0] == '-')
        {
            ...
            else if (strcmp(z, "debug") == 0)
            {
                if (i == argc - 1)
                    cmdlineError("missing argument to %s", argv[i]);
                g.fDebug = (unsigned)strtol(argv[++i], 0, 0);
            }
...
#ifndef SQLITE_OMIT_LOAD_EXTENSION
            else if (strcmp(z, "lib") == 0 || strcmp(z, "L") == 0)
            {
                if (i == argc - 1)
                    cmdlineError("missing argument to %s", argv[i]);
