...
void test_xfile_tmp(void)
{
    ...
    EQ(xfile_tmp_open(&file), DCP_OK);
    EQ(fwrite("12345", 5, 1, file.fp), 1);
    EQ(fflush(file.fp), 0);
    rewind(file.fp);

    uint64_t hash = 0;
    EQ(xfile_hash(file.fp, &hash), DCP_OK);
    EQ(hash, 14335752410685132726ULL);
...
void test_xfile_set_ext(void)
{
    ...
    EQ(xfile_set_ext(sizeof path, path, ".pdf"), DCP_ENOMEM);
    EQ(path, "file");

    EQ(xfile_set_ext(sizeof path0, path0, ".pdf"), DCP_OK);
    EQ(path0, "file0.pdf");

    EQ(xfile_set_ext(sizeof path1, path1, ".doc"), DCP_OK);
    EQ(path1, "file1.doc");

    EQ(xfile_set_ext(sizeof path2, path2, ".o"), DCP_OK);
