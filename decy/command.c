...
static void fn_scanny_polling(struct msg *msg) { (void)msg; }
...
static void fn_exec_scan_job_1(struct msg *msg)
{
    ...
    broker_send(SCHEDY_ID,
                fmt("job_set_state %lld run | exec_scan_job_2 {1} %lld", job_id,
                    job_id));
}
...
static void fn_exec_scan_job_3(struct msg *msg)
{
    ...
    broker_send(SCHEDY_ID,
                fmt("scan_dl_seqs %lld seqs.json | exec_scan_job_4 {1} {2} %s",
                    scan.id, buffer));
}
...
{
    debug("%s", msg_unparse(msg));
    if (msg_check(msg, "sssss")) return;
    if (strcmp(msg_argv(msg)[1], "ok")) return;
    if (!broker_parse_scan(&scan, msg_argv(msg)[2])) return;
    char *seqs_file = msg_argv(msg)[3];
    char *db_file = msg_argv(msg)[4];
    broker_send(SCANNY_ID, fmt("scan %s %s prods.tsv %d %d", seqs_file, db_file,
                               scan.multi_hits, scan.hmmer3_compat));
}
...
static void fn_exec_press_job_1(struct msg *msg)
{
    ...
    int64_t job_id = msg_int(msg, 3);
    broker_send(SCHEDY_ID,
                fmt("job_set_state %lld run | exec_press_job_2 {1} %lld",
                    job_id, job_id));
}
