...
#define CMD_MAP(X)                                                             \
    ...
    X(UPDATE_SCAN_PROGRESS_3, update_scan_progress_3, "TODO")                  \
...
static long filename_id(char *name)
{
...
#undef X
    ...
    broker_send(PARENT_ID, cmd_help_table());
}
...
static void fn_schedy_polling(struct msg *msg)
{
    ...
    if (type == SCHED_SCAN)
        broker_send(SCANNY_ID,
                    fmt("state | exec_scan_job_1 {1} {2} %ld", job.id));

    if (type == SCHED_HMM)
        broker_send(PRESSY_ID,
                    fmt("state | exec_press_job_1 {1} {2} %ld", job.id));
}
...
static void fn_update_scan_state_2(struct msg *msg)
{
    ...
    char const *state = msg_argv(msg)[3];
    long scan_id = filename_id(filename);
    broker_send(SCHEDY_ID,
                fmt("scan_get_by_id %ld | update_scan_state_3 {1} {2} %s %s",
                    scan_id, filename, state));
}
...
static void fn_update_scan_state_3(struct msg *msg)
{
    ...
    long job_id = scan.job_id;
    char *filename = msg_argv(msg)[3];
    char const *state = msg_argv(msg)[4];
...
static void fn_update_press_state_3(struct msg *msg)
{
    ...
    char const *state = msg_argv(msg)[3];
    if (!strcmp(state, "done"))
        broker_send(SCHEDY_ID, fmt("job_inc_progress %ld 100", hmm.job_id));
    broker_send(SCHEDY_ID,
...
static void fn_update_scan_progress_2(struct msg *msg)
{
    ...
    long scan_id = filename_id(filename);
    broker_send(SCHEDY_ID,
                fmt("scan_get_by_id %ld | update_scan_progress_3 {1} {2} %d",
                    scan_id, inc));
}

static void fn_update_scan_progress_3(struct msg *msg)
{
    if (msg_check(msg, "ss*")) return;
    if (strcmp(msg_argv(msg)[1], "ok")) return;
    if (msg_check(msg, "sssi")) return;
    ...
    if (!broker_parse_scan(&scan, json)) return;
    int inc = (int)msg_int(msg, 3);
    broker_send(SCHEDY_ID, fmt("job_inc_progress %ld %d", scan.job_id, inc));
}
...
static void fn_update_press_progress_3(struct msg *msg)
{
    ...
    int inc = (int)msg_int(msg, 3);
    if (!broker_parse_hmm(&hmm, json)) return;
    broker_send(SCHEDY_ID, fmt("job_inc_progress %ld %d", hmm.job_id, inc));
}
...
static void fn_exec_scan_job_2(struct msg *msg)
{
    ...
    long job_id = msg_int(msg, 2);
    broker_send(
        SCHEDY_ID,
        fmt("scan_get_by_job_id %ld | exec_scan_job_3 {1} {2}", job_id));
}
...
static void fn_exec_scan_job_3(struct msg *msg)
{
    ...
    broker_send(
        SCHEDY_ID,
        fmt("scan_dl_seqs %ld seqs_%ld.json | exec_scan_job_4 {1} {2} %s",
...
static void fn_exec_scan_job_4(struct msg *msg)
{
    ...
    sched_dump_scan(&scan, buffer);
    broker_send(SCHEDY_ID,
                fmt("db_get_by_id %ld | exec_scan_job_5 {1} {2} %s %s",
...
static void fn_exec_scan_job_5(struct msg *msg)
{
    ...
    broker_send(SCHEDY_ID,
                fmt("db_dl %ld %s | exec_scan_job_6 {1} %s %s %s", db.xxh3,
...
static void fn_exec_scan_job_6(struct msg *msg)
{
    ...
    broker_send(SCANNY_ID,
                fmt("scan %s %s %ld_prods.tsv %d %d", seqs_file, db_file,
                    scan.id, scan.multi_hits, scan.hmmer3_compat));
}
...
static void fn_exec_press_job_2(struct msg *msg)
{
    ...
    broker_send(
        SCHEDY_ID,
        fmt("hmm_get_by_job_id %ld | exec_press_job_3 {1} {2}", job_id));
}
...
static void fn_exec_press_job_3(struct msg *msg)
{
    ...
    broker_send(SCHEDY_ID, fmt("hmm_dl %ld %s | exec_press_job_4 {1} %s",
