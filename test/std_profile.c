...
void test_std_profile(void)
{
    ...
    struct dcp_std_profile *prof1 =
        dcp_std_profile_new(&m->abc, dcp_meta("NAME1", "ACC1"));
    imm_hmm_reset_dp(m->null.hmm, imm_super(state), prof1->dp.null);
    imm_hmm_reset_dp(m->hmm, imm_super(state), prof1->dp.alt);
    imm_del(hmm);
    imm_del(state);

    dcp_std_profile_del(prof0);
    dcp_std_profile_del(prof1);
    imm_example1_deinit();
}
